"""Evaluate person ROI extraction with a large-person fallback rule."""
...
def select_primary_person(
    ...
    This experiment intentionally keeps the same person-selection rule as
    earlier ROI experiments and chooses the highest-confidence person.
...
def select_roi_strategy(
    bbox_area_ratio: float,
) -> tuple[str, float]:
    """Select the ROI strategy and margin from person scale.

    Args:
        bbox_area_ratio: Person bounding-box area divided by image area.

    Returns:
        Strategy name and margin ratio.

    Notes:
        When the person bbox already occupies more than 85% of the image,
        this experiment stops expanding the bbox and uses margin 0.
    """
    if bbox_area_ratio > FULL_FRAME_THRESHOLD:
        return "large_person_no_expand", FULL_FRAME_MARGIN

    if bbox_area_ratio < SMALL_BBOX_THRESHOLD:
        return "adaptive_margin", SMALL_BBOX_MARGIN

    if bbox_area_ratio < LARGE_BBOX_THRESHOLD:
        return "adaptive_margin", MEDIUM_BBOX_MARGIN

    return "adaptive_margin", LARGE_BBOX_MARGIN
...
def evaluate_image(
    ...
    """Evaluate person ROI extraction with large-person fallback.
    ...
    Returns:
        Summary row describing the selected person and ROI.
    """
    ...
    roi_strategy, margin_ratio = select_roi_strategy(bbox_area_ratio)
    ...
    overlay_path = output_dir / f"{image_path.stem}_fallback_overlay.jpg"
    save_image(
        overlay_path,
        overlay,
    )
    ...
    roi_path = output_dir / f"{image_path.stem}_fallback_roi.jpg"
    save_image(
        roi_path,
        roi,
    )
    ...
    LOGGER.info(
        "Person score=%.4f, bbox ratio=%.4f, "
        "strategy=%s, margin=%.2f, expanded bbox ratio=%.4f",
        person_score,
        bbox_area_ratio,
        roi_strategy,
        margin_ratio,
...
def main() -> None:
    """Run person ROI evaluation with large-person fallback."""
    configure_logging()
    ...
    output_dir = project_root / "outputs" / "person_roi_full_frame_fallback_batch"
    ...
    fallback_count = sum(
        row["roi_strategy"] == "large_person_no_expand" for row in summary_rows
    )

    LOGGER.info(
        "Fallback ROI evaluation completed: %d/%d successful",
        success_count,
        len(summary_rows),
    )
    LOGGER.info(
        "Large-person fallback triggered: %d",
        fallback_count,
    )
    LOGGER.info(
        "Results saved to: %s",
