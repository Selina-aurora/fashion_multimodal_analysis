...
3. local_enlarged
   Run Grounding DINO on deterministic target-conditioned tight local windows
   inside the garment ROI. By default no manual interpolation enlargement is
   applied; the tighter crop makes the target occupy a larger fraction of the
   model input after normal processor resizing. Detections are mapped back to
   the original image and duplicate boxes are merged.
...
LOCAL_WINDOW_RULES = {
    # Sleeve:
    # Use two narrower side windows. These retain some upper-body context
    # while removing most of the central garment region.
    "sleeve": [
        (0.00, 0.02, 0.36, 0.75),
        (0.64, 0.02, 1.00, 0.75),
    ],
    ...
    # Collar:
    # Focus on the upper-central garment region.
    "collar": [
        (0.20, 0.00, 0.80, 0.34),
    ],

    # Button:
    # Scan the central front area with three overlapping narrow windows.
    # This avoids assuming that the placket is perfectly centered.
    "button": [
        (0.28, 0.06, 0.50, 0.94),
        (0.39, 0.06, 0.61, 0.94),
        (0.50, 0.06, 0.72, 0.94),
    ...
    # Zipper:
    # Similar three-window scan, slightly taller because zippers can span
    # a large vertical portion of the garment.
    "zipper": [
        (0.28, 0.03, 0.50, 0.97),
        (0.39, 0.03, 0.61, 0.97),
        (0.50, 0.03, 0.72, 0.97),
