"""Evaluate person ROI extraction on a batch of DeepFashion2 images."""
...
def get_label_name(
    ...
    """Resolve a class ID to a human-readable model label.
...
def find_person_segments(
    ...
    Args:
        segments_info: Metadata for predicted instances.
        id2label: Model class ID to label mapping.
...
def select_primary_person(
    ...
    The current experiment selects the person with the highest confidence.
    This is an initial rule and is not considered the final multi-person
    selection strategy.

    Args:
        person_segments: Predicted person segments.
...
def calculate_bbox_area_ratio(
    ...
    Returns:
        Bounding-box area divided by total image area.
    """
    x_min, y_min, x_max, y_max = bbox
...
def build_empty_summary_row(
    ...
    """Build a summary row for a failed ROI extraction case.
...
def save_summary(
    ...
    """Save ROI evaluation summary rows to CSV.
...
def evaluate_image(
    ...
    """Evaluate person ROI extraction for one image.
    ...
    Returns:
        One summary row describing the selected person and ROI.
    """
    ...
    overlay_path = output_dir / f"{image_path.stem}_expanded_overlay.jpg"
    save_image(
        overlay_path,
        overlay,
    )
    ...
    expanded_roi_path = output_dir / f"{image_path.stem}_expanded_roi.jpg"
    save_image(
        expanded_roi_path,
        expanded_roi,
    )
    ...
    LOGGER.info(
        "Person score=%.4f, bbox ratio=%.4f, " "expanded bbox ratio=%.4f",
        person_score,
        bbox_area_ratio,
        expanded_bbox_area_ratio,
    )
...
def main() -> None:
    """Run batch person ROI evaluation."""
    configure_logging()
    ...
    output_dir = project_root / "outputs" / "person_roi_margin_batch"
    ...
    LOGGER.info(
        "Batch ROI evaluation completed: %d/%d successful",
        success_count,
        len(summary_rows),
    )
    LOGGER.info(
